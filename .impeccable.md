...
### 1. Typography lock (resolved: reopened for dial calibration)

The Outfit + DM Sans lock in CLAUDE.md is reopened. Resolution is a structured selection process at the dial-calibration step, governed by the following ground rules.

**Pressure on the old lock.** The document makes three independent commitments that put pressure on the Outfit + DM Sans pairing:

- **"Editorial"** as a brand adjective (Q2) commits us to magazine-style typographic treatment.
...
**Ground rules for the dial-calibration presentation.**

1. **Three options, not two.** A binary presentation forces a false choice; a trinary presentation lets the owner triangulate.
2. **Each option is rendered on a real hero mock of the homepage, not an isolated specimen card.** Typography is judged in context, not on a foundry-style specimen sheet. The owner sees each pairing carrying the actual hero headline, the subhead, the first paragraph of supporting copy, a CTA label, a caption, and a micro-label — in situ, on the real layout.
3. **Inter and Geist are both banned** per anti-reference 3 and Taste Skill rules. Do not propose either.
4. **Composition mandate for the three options:**
   - **At least one option** must be an editorial serif paired with a modernist sans (Gentlewoman-coded). Candidates to consider: a contemporary display serif (GT Sectra, Canela, Tiempos Headline, Reckless, Söhne Breit) paired with a precise grotesk (Söhne, GT America, ABC Diatype, National 2).
   - **At least one option** must be a single workhorse sans with enough range — display through micro — to carry the whole site. Candidates: Söhne family, GT America family, ABC Diatype family. Sibling serif or mono permitted only for long-read body or UI affordances.
   - **The third option** is the designer's wildcard — can be a left-field pairing that makes a stronger claim on the "a little dry" adjective than the first two, provided it still serves all five.
5. **Each option ships with its reasoning**, structured as a short brief:
   - *What it does for Melissa's trust signal.* The specific way this pairing makes her read the site as quiet, considered, and not a personal-injury-mill relabel.
   - *How it serves each of the five adjectives.* Plainspoken / Considered / Unhurried / Editorial / A little dry, graded against the pairing.
   - *What it rules out.* The aesthetics this pairing makes impossible (e.g., "this pairing cannot be mistaken for a Squarespace template"). Explicit ruling-out is more useful than ruling-in.
6. **Licensing and performance costs are disclosed.** Each option notes whether it is Google Fonts, self-hosted OTF/WOFF2 with a licensing fee, or a variable font. Font-file weight and CLS implications are part of the pick.
7. **Owner picks one, remixes one, or rejects all three.** If all three are rejected, we iterate. We do not silently retain Outfit + DM Sans as a fallback. The old lock is dead the moment the reopen is approved, which it now is.
